<?php
// conexão com o BD
$mysqli = mysqli_connect('localhost', 'root', '0719', 'Cursos');

// buscar valores no BD
$columns = array('carga_horaria','titulo','descricao');
...
if ($result = $mysqli->query('SELECT * FROM Cursos ORDER BY ' .  $column . ' ' . $sort_order)) {
	...
	<html>
		...
		<body>
			<table>
				<tr>
					<th><a href="index.php?column=carga_horaria&order=<?php echo $asc_or_desc; ?>">Carga horaria<i class="fas fa-sort<?php echo $column == 'carga_horaria' ? '-' . $up_or_down : ''; ?>"></i></a></th>
					...
					<th><a href="index.php?column=descricao&order=<?php echo $asc_or_desc; ?>">Descriçao <i class="fas fa-sort<?php echo $column == 'descricao' ? '-' . $up_or_down : ''; ?>"></i></a></th>
				</tr>
